...
object ReaderApp extends App {
  ...
  object ReaderService {
    def transact(currency: Currency, good: Good): Reader[Exchanges, Transaction] =
      Reader.ask[Exchanges, Transaction](e => Transaction(currency, good.price * e(good.currency, currency)))

    def aggregate(currency: Currency, transactions: NonEmptyList[Transaction]): Reader[Exchanges, Transaction] =
      Reader.ask[Exchanges, Transaction](e =>
        Transaction(
          currency = currency,
          price = transactions.map(f => f.price * e(f.currency, currency)).reduce(_ + _)
        )
      )
